    ...
    <div className="bg-mint-light w-full min-h-full pb-20 pt-20">
      <div className="bg-mint h-16 flex flex-row items-center justify-between px-4 fixed top-0 left-0 w-full z-50">
        <div className="flex flex-row items-center gap-2">
          <div onClick={()=>setSidebar(true)} className="rounded-full bg-white w-10 h-10 flex items-center justify-center font-bold">
            {wallet?.name[0]}
          </div>
      ...
      <div className={`fixed left-0 top-0 bottom-0 bg-gray-200 w-20 z-50 flex flex-col justify-between transform-gpu transition-all ${sidebar?'':'-ml-20'}`}>
        <div className="flex flex-col justify-center items-center">
          <div className="w-10 h-10 mt-3 mb-3 bg-white flex items-center justify-center rounded-full" onClick={()=>setSidebar(false)}>
            ✖
          </div>
        {
          wallets?.map((_: any, index: number) => (
            <div onClick={()=>{selectWallet(index);setSidebar(false);navigate('/wallet');}} key={index} className="flex flex-col items-center justify-center h-20 w-20">
        ...
        <div className="flex flex-col justify-center items-center mb-4 gap-4 mx-3 border-t-4 border-t-mint-dark pt-6">
          ...
          <Link to="/start" className="rounded-2xl bg-white flex flex-col items-center justify-center h-10 w-10">➕</Link>
        </div>
      </div>

      {
        isTestnet && (
          <div className="-mt-4 mb-4 text-center font-mono fixed left-0 right-0 top-20 z-40">
            ...
            <div className="text-black bg-amber-200 relative -top-2 px-2 rounded-b-xl w-auto inline-block">testnet mode</div>
          </div>
        )
      }
      ...
      <div className="fixed left-0 bottom-0 w-full bg-mint justify-around flex items-center pb-safe pt-1">
        <div className="my-2 justify-around flex items-center w-full">
          <NavLink end className={({ isActive }) => `${isActive?'bg-mint-light':'bg-mint'} rounded-xl px-2 py-2`} to="/wallet">
            Home
          </NavLink>
          <NavLink className={({ isActive }) => `${isActive?'bg-mint-light':'bg-mint'} rounded-xl px-2 py-2`} to="/wallet/dex">
            Swap
          </NavLink>
          {/*<NavLink className={({ isActive }) => `${isActive?'bg-mint-light':'bg-mint'} rounded-xl px-2 py-2`} to="/wallet/pools">*/}
          {/*  Staking <sup>soon</sup>*/}
          {/*</NavLink>*/}
          <NavLink className={({ isActive }) => `${isActive?'bg-mint-light':'bg-mint'} rounded-xl px-2 py-2`} to="/wallet/activity">
            Activity
          </NavLink>
        </div>
      </div>
